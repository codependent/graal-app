...
```bash
aws lambda create-function --function-name graal-app \
...
```bash
aws lambda invoke --function-name graal-app --payload '{"resource": "/{proxy+}", "path": "/ping", "httpMethod": "GET"}' build/response.txt
